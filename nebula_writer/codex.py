...
class CodexDatabase:
    ...
    def _insert_default_templates(self):
                ...
                "structure": json.dumps(
                    {
                        "acts": [
                            ...
                            },
                            {
                                ...
                                "beats": ["Tests, allies, enemies", "Ordeal", "Reward (seizing the sword)"],
                            },
                            {
                                "name": "Act 3 - Return",
                                "beats": ["The road back", "Resurrection", "Return with elixir"],
                            },
                        ]
                    }
                ),
                ...
                "structure": json.dumps(
                    {
                        "acts": [
                            {
                                "name": "Act 1",
                                "beats": ["Opening image", "Theme stated", "Set-up", "Catalyst", "Debate"],
                            },
                ...
                "structure": json.dumps(
                    {
                        "acts": [
                            ...
                            },
                            {
                                "step": 2,
                                "name": "One Paragraph",
                                "description": "Expand to a one-paragraph summary including setup, 3 disasters, ending",
                            },
                            {
                                "step": 3,
                                "name": "Character Synopses",
                                "description": "Create character synopses for each major character",
                            },
                            {
                                "step": 4,
                                "name": "Four Pages",
                                "description": "Expand each sentence of the paragraph into a paragraph",
                            },
        ...
        for _key, template in templates.items():
            cursor.execute(
                """
    ...
    def search(self, query: str) -> Dict:
        ...
        results = {"entities": [], "chapters": [], "events": []}
    ...
    def run_consistency_check(self) -> List[Dict]:
        ...
        # Optimization: Pre-compute lowercased chapter contents to avoid repeated O(N) string processing
        lower_chapter_contents = [c.get("content", "").lower() if c.get("content") else "" for c in chapters]

        for entity in entities.values():
            found = False
            entity_name_lower = entity["name"].lower()
            for chapter_content_lower in lower_chapter_contents:
            ...
            if count >= 2:
                ...
                for word in words:
                    if word in context:
                        # Simple heuristics
                        if any(
                            loc in context for loc in ["city", "street", "house", "room", "building", "town", "village"]
                        ):
