    ...
    def search_all(self, query: str) -> Dict:
        ...
        for e in self.db.get_entities():
            if (query in e['name'].lower() or 
                query in (e.get('description') or '').lower()):
                e['attributes'] = self.db.get_attributes(e['id'])
                results["entities"].append(e)
        
        # Search chapters
        for ch in self.db.get_chapters():
            if (query in (ch.get('title') or '').lower() or 
                query in (ch.get('content') or '').lower() or
                query in (ch.get('summary') or '').lower()):
                results["chapters"].append(ch)
        
        # Search events
        for ev in self.db.get_events():
            if (query in ev['title'].lower() or 
                query in (ev.get('description') or '').lower()):
                results["events"].append(ev)
        
        # Search relationships
        for rel in self.db.get_relationships():
            if (query in rel.get('relationship_type', '').lower() or
                query in (rel.get('description') or '').lower()):
                results["relationships"].append(rel)
        
        return results
    
    def filter_entities(self, entity_type: str = None, 
