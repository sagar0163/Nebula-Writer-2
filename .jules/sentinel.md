...
## 2026-07-04 - [Fix Overly Permissive CORS Configuration]
**Vulnerability:** The FastAPI application used a wildcard `allow_origins=["*"]` while having `allow_credentials=True`. This is a significant security risk (CSRF/data read vulnerabilities) as it allows any website to make credentialed requests to the API.
**Learning:** Using `allow_origins=["*"]` with `allow_credentials=True` is often blocked by modern browsers but represents a dangerous pattern that can lead to cross-site request forgery and data exfiltration.
**Prevention:** Always use an environment variable (e.g., `ALLOWED_ORIGINS`) with a safe local fallback to strictly govern allowed origins, especially when credentials are permitted.
