...
class EvolvingOutlineEngine:
    ...
    def _detect_tensions_from_prose(self, content: str):
        ...
        # ⚡ Bolt: Cache lowercased descriptions to avoid O(N*M) string re-allocations
        existing_tensions_lower = [t.description.lower() for t in self.open_tensions]

        for pattern_name, keywords, priority in tension_patterns:
            for keyword in keywords:
                if keyword in content_lower and not any(pattern_name in t_desc for t_desc in existing_tensions_lower):
                    self.add_open_tension(
                        description=f"{pattern_name}: {keyword}", chapter=self.current_chapter, priority=priority
                    )
                    # Also add to our cache so we don't detect the same one twice in this pass
                    existing_tensions_lower.append(f"{pattern_name}: {keyword}".lower())
                    break
    ...
    def _detect_seeds_from_prose(self, content: str):
        ...
        # ⚡ Bolt: Cache lowercased content to avoid O(N*M) string re-allocations
            ...
            for pattern in seed_patterns:
                if pattern in sentence_lower and len(sentence) < 100:
                    ...
                    if not any(pattern in s_content for s_content in planted_seeds_lower):
                        ...
                        # Also add to our cache so we don't plant the same seed twice in this pass
                        planted_seeds_lower.append(sentence.strip()[:100].lower())
