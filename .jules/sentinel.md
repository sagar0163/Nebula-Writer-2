...
## 2024-06-06 - [Hardcoded API Keys in `nebula_writer/run_supabase.py`]
**Vulnerability:** A hardcoded `SUPABASE_URL` and `SUPABASE_ANON_KEY` were present in `nebula_writer/run_supabase.py`.
**Learning:** Hardcoding API keys in utility scripts is a critical security vulnerability that exposes access to external infrastructure.
**Prevention:** Load credentials via `.env` files or system environment variables instead of committing secrets into source control.
