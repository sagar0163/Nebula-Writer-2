...
class EvolvingOutlineEngine:
    ...
    def _detect_tensions_from_prose(self, content: str):
        ...
        # Cache lowercase strings to prevent O(N*M) allocation overhead in the iteration below
        ...
        for pattern_name, keywords, priority in tension_patterns:
            for keyword in keywords:
                if keyword in content_lower and not any(
                    pattern_name in t_desc_lower for t_desc_lower in open_tensions_lower
                ):
                    new_tension_desc = f"{pattern_name}: {keyword}"
                    self.add_open_tension(description=new_tension_desc, chapter=self.current_chapter, priority=priority)
                    open_tensions_lower.append(new_tension_desc.lower())
                    break
        ...
        for sentence in sentences:
            ...
            for pattern in seed_patterns:
                if pattern in sentence_lower and len(sentence) < 100:
                    ...
                    if not any(pattern in s_lower for s_lower in planted_seeds_lower):
