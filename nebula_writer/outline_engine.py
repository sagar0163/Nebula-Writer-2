...
class EvolvingOutlineEngine:
    ...
    def _detect_tensions_from_prose(self, content: str):
        ...
        # ⚡ OPTIMIZATION: Cache lowercased descriptions outside the loop to prevent O(N*M) string allocation overhead
        existing_tensions = [t.description.lower() for t in self.open_tensions]

        for pattern_name, keywords, priority in tension_patterns:
            # ⚡ OPTIMIZATION: Check pattern presence before inner loop
            if any(pattern_name in desc for desc in existing_tensions):
                continue
            ...
            for keyword in keywords:
                if keyword in content_lower:
                    ...
                    existing_tensions.append(new_desc.lower())
                    break
    ...
    def _detect_seeds_from_prose(self, content: str):
        ...
        # ⚡ OPTIMIZATION: Cache lowercased seed contents outside the loop
        existing_seeds = [s.content.lower() for s in self.planted_seeds]
        ...
        for sentence in sentences:
            ...
            for pattern in seed_patterns:
                if pattern in sentence_lower and len(sentence) < 100:
                    ...
                    if not any(pattern in desc for desc in existing_seeds):
                        new_seed = sentence.strip()[:100]
                        self.add_planted_seed(new_seed, self.current_chapter)
                        # ⚡ OPTIMIZATION: Keep cache updated to prevent re-adding if same pattern occurs multiple times
                        existing_seeds.append(new_seed.lower())
