...
class NarrativeOrchestrator:
    ...
    def __init__(self):
        ...
        # 4. Initialize style learning (after AI and DB are ready)
        self.ai.style_learner = create_style_learner(self.db)
        self.ai.style_learner_db = self.db

        # 5. Initialize spatial mapping (after DB is ready)
        self.spatial_mapper = create_spatial_mapper(self.db)
    ...
    async def handle_write_scene(self, beat: str) -> Dict:
        ...
        constraints = GenerationConstraints(
            directive=directive, character_predictions=character_predictions, context_string=context
        )
        ...
        prose = await self.ai._generate(
            system_prompt, f"Write the scene for this beat: {beat}", role="writer", max_tokens=4000
        )
        ...
        return {
            "prose": prose,
            "directive": {"outcome": directive.required_outcome, "pacing": directive.pacing_target},
            "alignment": alignment,
            "integrity": {"is_valid": validation["is_valid"], "violations": validation["violations"]},
        }
