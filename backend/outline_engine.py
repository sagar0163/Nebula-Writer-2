...
class EvolvingOutlineEngine:
    ...
    def _generate_scene_intent(self) -> str:
        ...
        if self.open_tensions:
            highest = max(self.open_tensions, key=lambda t: 
                {"critical": 4, "high": 3, "normal": 2, "low": 1}.get(t.priority, 2))
    ...
    def _get_arc_pressure_character(self) -> str:
        ...
        oldest = min(self.character_arc_states.values(), 
                    key=lambda a: a.chapter_last_updated if a.chapter_last_updated else 0)
    ...
    def add_open_tension(self, description: str, chapter: int = None, 
                        priority: str = "normal", characters: List[str] = None) -> str:
    ...
    def update_character_arc(self, character_id: str, character_name: str,
                             belief: str = None, want: str = None,
                             knows: List[str] = None, emotional: str = None) -> None:
...
if __name__ == "__main__":
    ...
    compass = engine.initialize({
        "emotional_start": "burdened",
        "midpoint_disruption": "discovers truth",
        "ending_resolution_type": "ambiguous"
    })
    
    print(f"\nStory Compass: {json.dumps(compass, indent=2)}")
    
    # Add open tension
    tid = engine.add_open_tension(
        "Who killed the crime lord?", 
        priority="high"
    )
    print(f"\nAdded tension: {tid}")
