...
## 2024-06-09 - [Remove Hardcoded Supabase Tokens]
**Vulnerability:** A hardcoded `SUPABASE_ANON_KEY` and `SUPABASE_URL` existed in `nebula_writer/run_supabase.py`.
**Learning:** Developer scripts meant for testing often inadvertently hardcode sensitive credentials and get committed.
**Prevention:** Never commit scripts containing real credentials. Use `.env` files or require environment variables to be set before execution.
