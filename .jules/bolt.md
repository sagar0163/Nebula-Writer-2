...
## 2024-06-27 - Keeping cached arrays synchronized when modifying underlying lists
**Learning:** When applying performance optimizations like caching string transformations outside of a loop to avoid O(N*M) allocation overhead, it is critical to keep the cache synchronized if the underlying list is mutated during the loop. In `outline_engine.py`, new tensions and planted seeds were being appended inside the loop. If the local cache (`open_tensions_desc_lower` or `planted_seeds_lower`) was not also updated, newly added elements would not be evaluated in subsequent loop iterations, leading to functional regressions.
**Action:** Always check if the underlying lists or structures are mutated during the iteration (e.g., appending new items). If so, explicitly update the cache within the loop to keep it synchronized and prevent functional regressions due to stale state.
