...
class EvolvingOutlineEngine:
    ...
    def _detect_tensions_from_prose(self, content: str):
        ...
        # ⚡ Bolt: Cache lowercased tension descriptions to prevent O(N*M) allocation overhead in generator expression
        open_tensions_lower = {t.description.lower() for t in self.open_tensions}

        for pattern_name, keywords, priority in tension_patterns:
            for keyword in keywords:
                if keyword in content_lower and not any(pattern_name in t_desc for t_desc in open_tensions_lower):
                    new_desc = f"{pattern_name}: {keyword}"
                    self.add_open_tension(description=new_desc, chapter=self.current_chapter, priority=priority)
                    # ⚡ Bolt: Keep cache synchronized when underlying list is mutated
                    open_tensions_lower.add(new_desc.lower())
                    break
    ...
    def _detect_seeds_from_prose(self, content: str):
        ...
        # ⚡ Bolt: Cache lowercased planted seed contents to prevent O(N*M) allocation overhead in generator expression
            ...
            for pattern in seed_patterns:
                if pattern in sentence_lower and len(sentence) < 100:
                    ...
                    if not any(pattern in s_content for s_content in planted_seeds_lower):
                        ...
                        # ⚡ Bolt: Keep cache synchronized when underlying list is mutated
                        planted_seeds_lower.add(new_content.lower())
