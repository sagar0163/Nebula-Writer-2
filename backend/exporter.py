...
"""
    ...
    def to_epub(self, title: str = "My Novel", author: str = "Author", 
...
<container version="1.0" xmlns="urn:oasis:names:tc:opendocument:xmlns:container">
  <rootfiles>
    <rootfile full-path="OEBPS/content.opf" media-type="application/oebps-package+xml"/>
            ...
            manifest_items = []
            nav_id = "nav"
            manifest_items.append(f'    <item id="{nav_id}" href="nav.xhtml" media-type="application/xhtml+xml" properties="nav"/>')
            ...
            for i, ch in enumerate(sorted(chapters, key=lambda x: x['number'])):
                ch_id = f"ch{i+1}"
                manifest_items.append(f'    <item id="{ch_id}" href="chapter_{ch_id}.xhtml" media-type="application/xhtml+xml"/>')
...
'''
            ...
            for i, ch in enumerate(sorted(chapters, key=lambda x: x['number'])):
                ...
                para_html = '\n'.join([f'<p class="{"first" if j == 0 else ""}">{self._escape_xml(p)}</p>' 
                                     for j, p in enumerate(paragraphs) if p.strip()])
