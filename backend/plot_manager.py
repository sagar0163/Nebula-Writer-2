...
# Character Voice Profiles
    ...
    def add_plot_thread(self, title: str, description: str = None, 
                     planted_chapter: int = None, importance: str = "normal") -> int:
    ...
    def add_foreshadowing(self, plot_thread_id: int, chapter_id: int, content: str,
                       hint_level: str = "subtle", payoff_chapter: int = None) -> int:
    ...
    def add_world_rule(self, category: str, rule: str, description: str = None,
                   exceptions: str = None, applies_to: str = None) -> int:
        """Add a world rule"""
        conn = self._get_connection()
        cursor = conn.cursor()
        cursor.execute("""
    ...
    def set_voice_profile(self, entity_id: int, vocabulary_level: str = "average",
                        speech_patterns: str = None, common_phrases: str = None,
                        emotional_register: str = "neutral", formal_level: str = "neutral",
                        dialect: str = None, quirks: str = None, sample_dialogue: str = None) -> int:
        ...
        if existing:
            cursor.execute("""
                ...
                WHERE entity_id = ?
            """, (vocabulary_level, speech_patterns, common_phrases, emotional_register,
                  formal_level, dialect, quirks, sample_dialogue, datetime.now().isoformat(), entity_id))
            conn.commit()
            conn.close()
            return existing[0]
        else:
            cursor.execute("""
                ...
                VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)
            """, (entity_id, vocabulary_level, speech_patterns, common_phrases, emotional_register,
                  formal_level, dialect, quirks, sample_dialogue))
    ...
    def add_citation(self, topic: str, fact: str, source: str = None,
                  source_url: str = None, linked_entity_id: int = None,
                  linked_location: str = None) -> int:
    ...
    def check_continuity(self, chapter_id: int, chapter_text: str,
                       entities: List[Dict], events: List[Dict]) -> Dict:
    ...
    def calculate_narrative_momentum(self, open_tensions_count: int, 
                                   pacing_score: float, 
                                   chapters_since_major_event: int) -> float:
        """
...
if __name__ == "__main__":
    ...
    f_id = pm.add_foreshadowing(
        thread_id, 2, "A strange red glow appeared in the window",
        hint_level="subtle", payoff_chapter=8
    )
